...
std::vector<RandomPixel> generate_random_pixles(int total)
{
    std::srand(time(NULL));
    std::vector<RandomPixel> out;
    out.reserve(total);
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        RandomPixel pixel = {
            .pos = pos,
            .pixel = RGB(std::rand() % 256, std::rand() % 256, std::rand() % 256)
        };
...
int main()
{
    ...
    auto random_pixels = generate_random_pixles(200);
     ...
     while (1)
    {
        ...
        // display();
        for (auto& p: random_pixels)
        {
            setPixel(p.pos.x, p.pos.y, p.pixel);
        }

...
void cross_helper(Point origin, Uint32 color)
{
    setPixel(origin.x, origin.y, color);
    setPixel(origin.x, origin.y + 1, color);
    setPixel(origin.x, origin.y - 1, color);
    setPixel(origin.x + 1, origin.y, color);
    setPixel(origin.x + 1, origin.y + 1, color);
    setPixel(origin.x + 1, origin.y - 1, color);
    setPixel(origin.x - 1, origin.y, color);
    setPixel(origin.x - 1, origin.y + 1, color);
    setPixel(origin.x - 1, origin.y - 1, color);
...
void cross(Point origin, Uint32 color)
{
    cross_helper(origin, color);
    cross_helper({ origin.x + 3, origin.y }, color);
    cross_helper({ origin.x, origin.y + 3 }, color);
    cross_helper({ origin.x - 3, origin.y }, color);
    cross_helper({ origin.x, origin.y - 3 }, color);
}

void bezier(const std::vector<Point>& points, Uint32 color)
{
    if (points.size() != 4)
        return;

    for (float u = 0; u <= 1; u += 1e-3)
    {
        int xu = pow(1 - u, 3) * points[0].x + 3 * u * pow(1 - u, 2) * points[1].x + 3 * pow(u, 2) * (1 - u) * points[2].x + pow(u, 3) * points[3].x;
        int yu = pow(1 - u, 3) * points[0].y + 3 * u * pow(1 - u, 2) * points[1].y + 3 * pow(u, 2) * (1 - u) * points[2].y + pow(u, 3) * points[3].y;

        setPixel(xu, yu, color);

        // xu = (1-u)^3*x0+3*u*(1-u)^2*x1+3*u^2*(1-u)*x2+u^3*x3
        // yu = (1-u)^3*y0+3*u*(1-u)^2*y1+3*u^2*(1-u)*y2+u^3*y3
    }
}
...
int main()
{
     ...
     while (1)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if(event.type == SDL_MOUSEBUTTONDOWN)
            {
                ...
                if(event.button.button == SDL_BUTTON_LEFT)
				{
                    ...
                    bezier_pts.push_back({ event.motion.x, event.motion.y });
				}
            }
        }
        ...
        for (auto& point: bezier_pts)
        {
            cross(point, RGB(255, 0, 0));
        }

        bezier(bezier_pts, RGB(0, 255, 0));
